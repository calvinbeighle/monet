...
- ~~Spec 07: Front health tier boundary was at 50 instead of 25 for critical~~ - getHealthTier now returns degraded for scores 25-74, critical only below 25 per Spec 07
- ~~Spec 08: Arrow keys at strategic/tactical zoom with no selection selected nearest thread~~ - Added zoom level guard requiring operational/detail before first-arrow-key selection
- ~~Spec 10: Action queue never wrote succeeded/failed status before removal~~ - replayActionQueue now transitions to succeeded/failed before removing entries
- ~~Spec 11: visualExtent reset to 2 when cluster rebuilt, losing high-water mark~~ - Now preserves existingCluster.visualExtent as floor via Math.max
- ~~Spec 11: Non-participant affinity could produce composite above clustering threshold~~ - Added meetsThreshold flag requiring participantOverlap > 0
- ~~Spec 08: Unclustered thread dots visible at strategic zoom~~ - Individual unclustered threads now hidden at strategic zoom per Spec 08 visibility rules
- ~~TypeScript error in sync-engine.test.ts~~ - Simplified complex type assertion to direct Thread cast
