...
- ~~Spec 07: Risk tier had no visual presence on map canvas - renderer now uses riskTier for color shift (elevated=amber, critical=red, lost=gray) and forces pulsing at critical~~
- ~~Spec 03: onUserReply/onArchive drift functions never called in production - now wired into outbound-actions for immediate zone/targetPosition snap~~
- ~~Spec 07: getHealthTier returned degraded for 25-74 instead of 25-49 - threshold fixed to match spec (<50 = degraded)~~
- ~~Spec 07: Trust decay indicator not shown in UI - detail panel now shows decay arrow when decayActive is true~~
- ~~Spec 02: Age opacity used creation time instead of last activity time - now uses latestMessageTimestamp~~
- ~~Spec 08: Thread labels visible at tactical zoom - labels now suppressed at tactical per spec~~
- ~~Spec 11: Cluster membership removal was instant - now uses two-tick confirmation via pendingRemovals tracking~~
- ~~Spec 02: Unclustered threads hard-hidden at strategic zoom - now fade smoothly via zoomBlend~~
