...
## Status: Foundation + ingestion + AI core built. Visual design overhauled to Bridge Intelligence style. xAI Imagine integration complete. (4.0, 4.1, 4.2, 4.4 in progress).

Last audited: 2026-03-31 18:15 PDT

---

## CURRENT PRIORITY: Two separate layers

### Layer 1: Backend - Real Terminal Emulator

A fully capable terminal emulator backend. Not a toy. Real PTY processes that do real work.

Build (in order):

1. **WebSocket PTY server** - Python backend (extend `../agent/main.py` or standalone) using `pty` module. Spawns real shell processes (bash/zsh). Pipes stdin/stdout/stderr over WebSocket. Handles resize (SIGWINCH). Multiple concurrent sessions.
2. **Session management** - Create, list, kill, reconnect terminal sessions. Each session has an ID, a working directory, a process. Sessions persist across page reloads.
3. **Agent launcher** - Ability to launch specific agents in a session: `claude` CLI, custom scripts, any command. The terminal does real work - runs Claude Code, executes builds, manages git, whatever.
4. **Process lifecycle** - Track running/exited status, exit codes, send signals (SIGINT, SIGTERM). Clean up zombie processes.

### Layer 2: Frontend - TikTok Feed with AI-Generated Images/Videos

The frontend is a TikTok-style vertical swipe feed. Each card is a full-viewport visual generated by the **xAI Imagine API** (`https://api.x.ai`). NOT raw terminal output. Each workstream gets AI-generated imagery representing what the agent is working on.

**xAI Imagine API** - Key in 1Password ("xAI API Key"). Endpoint: `https://api.x.ai/v1/images/generations` (or check docs). Use it to generate visuals per workstream based on the agent's current task/context.

Build (in order):

1. [x] **xAI Imagine integration** - DONE - Frontend service at `src/lib/services/imagine-service.ts`, backend proxy at `/api/trace/imagine/generate` in `trace_router.py`. Prompts are auto-generated from workstream context (sources, urgency, name). Images cached with 30min TTL. Debounced 10s. Requires XAI_API_KEY env var on backend (key in 1Password 'xAI API Key', credential field).
2. **Video-style workstream cards** - Full-viewport snap-scroll cards. Each card shows the AI-generated image as a background/hero visual, with workstream name, status, and agent activity overlaid. Each card feels like a TikTok reel.
3. **Live activity overlay** - On top of the generated image: typing animations, progress indicators, status text, recent agent output snippets. The image is the "vibe", the overlay is the "info".
4. **Detail view = full terminal** - Tap into a card to expand. Shows the real terminal (xterm.js) for direct interaction with the agent. The feed shows the visual summary, the detail shows the raw terminal.
5. **Auto-refresh visuals** - Periodically summarize terminal output, regenerate the Imagine image when the workstream context shifts (new task, completed task, error state).

### Visual Design: Bridge Intelligence Style (Inverted/Dark Mode)

**IMPLEMENTED** - Fonts changed from DM Mono + Newsreader to Playfair Display (display/italic) + Crimson Text (body). Background changed to #000000. Neon green accent removed. Urgency colors updated to BI status palette (green `rgba(0,120,0,0.8)`, yellow `rgba(180,130,0,0.8)`, red `rgba(180,0,0,0.8)`). Source icon colors desaturated.

Follow the Bridge Intelligence brand guidelines at `/Users/calvinbeighle/BridgeIntelligence/BRAND-GUIDELINES.md`. Use the **inverted dark mode** variant:

**Typography** (both serif, never sans-serif):

- Display/headings: `"Playfair Display", Georgia, serif` - weight 400, italic
- Body/text: `"Crimson Text", serif` - weight 400 normal, 600 for strong
- Load via Google Fonts

**Colors (inverted Bridge Intelligence palette)**:

- Background: `#000000` (pure black, not the current #0a0a0b)
- Primary text: `rgba(255, 255, 255, 0.7)` (BI inverted body)
- Strong text: `rgba(255, 255, 255, 1.0)` (BI inverted emphasis)
- Muted: `rgba(255, 255, 255, 0.4)`
- Ghost: `rgba(255, 255, 255, 0.2)`
- Borders/rules: `rgba(255, 255, 255, 0.1)`
- Status only: green `rgba(0, 120, 0, 0.8)`, yellow `rgba(180, 130, 0, 0.8)`, red `rgba(180, 0, 0, 0.8)`
- NO blue, purple, orange, gradients, or bright accent colors
- NO neon green (#c8f06a) - remove this accent entirely

**Layout principles**:

- Separators: 40px wide, 1px - never full-width rules
- Insight blocks: italic Playfair at reduced opacity, thin left border
- Metrics: Playfair 1.3rem number + Crimson 0.8rem label below
- Hover: opacity 0.7, transitions 0.1s-0.3s ease-out
- No rounded corners larger than 3px
- No card-based dashboard layouts (each card IS the full viewport)
- No emoji, no icons, no stock photography
- Shadows max: `0 1px 4px rgba(0,0,0,0.1)`

**Voice**: Declarative, analytical, honest. Short paragraphs. No hype.

### What to SKIP for now:

- Gmail/Arc/Calendar/HubSpot/git ingestion - all deferred
- Fixture data - not needed

### Known bugs (already fixed, do not reintroduce):

- DO NOT call store methods inside Zustand selectors (causes infinite loops). Use raw state + useMemo.
- DO NOT use useCallback with Zustand actions in dependency arrays.
- DO NOT reference `--font-mono` or `--font-serif` CSS variables - they have been renamed to `--font-display` and `--font-body` in the Bridge Intelligence style overhaul.
...
- [x] **4.0 Design system + animation library** - Framer Motion (already installed), Google Fonts (DM Mono + Newsreader), CSS custom properties for design tokens (urgency colors, surfaces, typography), snap-scroll CSS classes, sparkline styles.
...
- [x] **4.4 Visual design spec** - DONE - Bridge Intelligence inverted dark mode implemented. Playfair Display + Crimson Text fonts, #000000 background, BI opacity scale for text, BI status colors for urgency, no neon green accent, no blue/purple/orange, rounded corners max 3px (using rounded-sm). - ~~Dark theme, cinematic feel. Near-black background (#0a0a0b). Urgency colors: red (urgent/overdue), amber (needs follow-up), green (on track), blue (informational). Typography: DM Mono for labels/metadata/timestamps, Newsreader (serif) for names/summaries/content. Cards have subtle depth (1px borders at 6% white opacity, hover lifts with translateY). Activity sparklines on each card (7 tiny bars showing last 7 days of activity). Scan-line overlay for CRT/operations-center feel. Animations: spring physics for card transitions (not linear easing), staggered reveals on feed load, parallax on scroll between cards. Command palette (Cmd+K) for power users.~~
...
25. **xAI API key provisioning** - The XAI_API_KEY environment variable must be set when running the backend for image generation. Key stored in 1Password ('xAI API Key', credential field). The frontend gracefully handles missing images (cards render without background). Consider adding the key to a `.env` file for the backend.
...
| 10-workstream-timeline       | 4.1, 4.1a             | In progress |
| 11-workstream-detail-view    | 4.2 (incl. merge UI)  | In progress |
