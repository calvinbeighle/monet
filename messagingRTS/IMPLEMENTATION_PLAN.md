...
- ~~Spec 09: sendReplyAction never called transitionState - lifecycle state did not change on reply~~ - applyReplyOptimisticUpdate now fires resolveTransition("user-replied") before optimistic update, enabling new->active, active->waiting, at-risk->active transitions
- ~~Spec 11: computeTopicOverlap used subject keywords only, ignoring body-derived topicTags~~ - Now uses thread.topicTags when available, falls back to extractKeywords(subject)
- ~~Spec 11: Cluster label used participant count >= 2 instead of requiring all members~~ - deriveLabel now requires bestCount >= threads.length for participant-based labels
- ~~Spec 01: approaching-archive state never completed to handled~~ - driftTick now fires archive-drift-complete transition when position converges (driftMagnitude < 1)
- ~~Spec 07: new-high-value alert required valueScore >= 0.7 not in spec~~ - Removed valueScore gating per Spec 07: alert fires on opportunity flag + window only
- ~~Spec 07: Trust decay thread-type lookup used first-encountered thread, not most recent~~ - runTrustDecay now sorts threads by latestMessageTimestamp descending
- ~~Spec 07: zeroLostDays streak checked current riskTier, not session tracking~~ - evaluateStreaks now accepts sessionLostCount parameter for accurate day-level tracking
- ~~Spec 12: Agent dock had no deployment history trigger~~ - AgentDock now accepts onHistoryClick prop, wired in App.tsx active state
- ~~Spec 10: Conflict notifications had no auto-dismiss~~ - Info-severity notifications auto-dismiss after 8 seconds per Spec 10 Section 6
