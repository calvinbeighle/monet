...
- Spec 09: Recomputation on restore does not recalculate cluster membership (requires full cluster evaluation pass, deferred)
- Spec 03: Topic tags extracted from subject+snippet only, not full message body (Spec 11 says "subject and body")
- Spec 04: Zone alert does not fire immediately on mount when count already outside threshold (only fires on first drift tick ~200ms later)

**Low**:

- Spec 02: Drift settlement timing not enforced (no 1-second guarantee)
- Spec 06: Results overlay not cluster-anchored (currently centered modal)
