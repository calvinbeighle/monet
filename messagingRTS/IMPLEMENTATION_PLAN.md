...
- Spec 06: Quick-deploy keyboard shortcut targets viewport center instead of selected cluster; no invalid-target affordance on failed shortcut - PARTIALLY FIXED: now targets cluster of selected thread, but no invalid-target visual affordance yet
- Spec 06: Context menu Deploy Agent submenu lists all agent types instead of filtering to valid types per cluster
...
- Spec 09: Contact enrichment not deduplicated once per session per participant (re-derives on every thread fetch)
- Spec 07: Idle auto-trigger fires session summary even when detail panel is open with active composer
- Spec 12: Session summary modal focus trap edge case - Tab from outside modal not intercepted on initial press
