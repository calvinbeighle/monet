...
- Spec 07: Trust increment table does not proportionally reflect latency tolerance ratios (internal=4 vs existing-relationship=5 is only a 1-point gap despite 6x tolerance difference)
- Spec 07: 3-consecutive trust indicator only shown in detail panel, not on map nodes across all that contact's threads
- Spec 07: No midnight scheduler for streak evaluation - streaks only increment during active sessions, not at midnight independently
- Spec 02: No scale indicator rendered in overlay layer (minimap exists but no scale bar)
- Spec 09: Lifecycle transition on reply not ordered before drift computation - both applied atomically
...
- Spec 06: Agent dock badge shows thread count instead of deployment count per role
- Spec 11: Override-excluded membership state not surfaced on thread record (only tracked in clustering module closure)
- Spec 01: Draft "discarded" state set but immediately deleted from map - never observable by consumers
- Spec 01: Sync engine does not proactively reduce poll frequency on near-exhaustion (reactive blocking only)

**Resolved (this round)**:

- ~~Spec 12: Thread selection on map did not update app-store - panel replacement (deployment-history -> detail) was broken because map-viewport only wrote to thread-store, not app-store~~
- ~~Spec 07: Agents never called flagOpportunity - Researcher (thread-context-summary) and Closer (reply/follow-up drafts) now flag threads as opportunities via evaluateOpportunityFlags~~
- ~~Spec 05: Escalation Bot had no end-to-end wiring - cluster now visually elevated (red pulsing ring) and critical notification fired per escalation-flag proposal~~
- ~~Spec 02: Drift settlement was not time-bounded - exponential approach alone left threads at 77% of distance after 1s; now forces snap at 1s boundary via targetSetTimes tracking~~
- ~~Spec 10: Conflict logs omitted action types - console.log now includes pendingActions.map(a => a.type) per spec requirement~~
...
### Implementation Notes - Spec Compliance Round 18 (2026-04-01)

- Risk tier visual rendering (Spec 07): map-renderer.ts now uses getRiskAwareColor() that maps riskTier to colors (elevated=amber, critical=red, lost=gray, safe=urgency-based). Critical risk tier forces pulse via riskPulseBoost.
- User action drift wiring (Spec 03): outbound-actions.ts now imports and calls driftOnUserReply/driftOnArchive for immediate zone and targetPosition updates on reply/archive.
- Health tier fix (Spec 07): getHealthTier threshold changed from >=75 to >=50 for healthy, matching spec's "<50 = degraded" requirement.
- Trust decay indicator (Spec 07): detail-panel.tsx ParticipantTrust now shows a red down-arrow when decayActive is true.
- Age opacity fix (Spec 02): map-renderer.ts uses latestMessageTimestamp (last activity) instead of firstMessageTimestamp for thread opacity decay.
- Tactical label suppression (Spec 08): Labels only render at operational/detail zoom, not tactical.
- Two-tick cluster removal (Spec 11): clustering.ts uses module-level pendingRemovals Set for two-tick confirmation before removing threads from clusters.
- Strategic fade (Spec 02): Unclustered threads at strategic zoom use zoomBlend-based alpha instead of visible=false.
- Fixed pre-existing game-loop test UTC/local date mismatch (toISOString vs local getFullYear/getMonth/getDate).
- Fixed pre-existing search-overlay lint error (missing searchResults.length in useCallback deps).
- Test count: 1228 total (10 new tests), all passing
