...
- ~~Spec 04: Zone alert does not fire immediately on mount when count already outside threshold~~ - Implemented immediate evaluateZoneAlerts + evaluateAlerts call before first drift tick interval
- ~~Spec 09: Recomputation on restore does not recalculate cluster membership~~ - loadPersistedThreads now calls evaluateClusters and applies threadUpdates; drift tick writes clusterMembership back to thread store
- ~~Spec 03: Topic tags extracted from subject+snippet only, not full message body~~ - extractKeywords now runs on concatenated bodyPlain from all thread messages, not just snippet
- ~~Spec 02: Drift settlement timing not enforced (no 1-second guarantee)~~ - driftTick snaps position to target when remaining distance < 1px, zeroes drift velocity
- ~~Spec 06: Results overlay not cluster-anchored (currently centered modal)~~ - ResultsOverlay computes cluster centroid screen position from deployment thread positions, anchors panel there
- ~~Spec 02: No label collision detection for thread entities~~ - Implemented circle-rectangle overlap check in renderThreads, hides labels that overlap adjacent thread glyphs
- ~~Spec 06: Map tooltip recall button not implemented~~ - Added interactive recall button to agent deployment tooltip on map
- ~~Spec 06: History panel recall option for in-progress entries not implemented~~ - Added recall button to traveling/in-progress deployment cards
- ~~Spec 08: Detail zoom level lacks reduced-opacity surrounding context~~ - Non-selected threads now rendered at alpha 0.3 in detail zoom
- ~~Spec 08: Minimap does not render cluster aggregate positions~~ - Minimap now renders cluster centroids as sized circles
- ~~Spec 11: Manual override exclusions not persisted across page reload (session-scoped)~~ - NOT a gap: spec explicitly says "for the current session" and puts cross-session persistence out of scope
- ~~Spec 09: Incomplete recomputation on restore - position, zone, drift velocity not recalculated~~ - loadPersistedThreads now recomputes targetPosition, zone (from actual position), and resets driftVelocity using computeTargetZone/computeTargetPosition/getZoneAtPosition
- ~~Spec 09: No removal of orphaned threads after Gmail sync~~ - startInitialLoad now reconciles thread store against Gmail thread IDs, removing orphaned entries per Spec 09 line 164
- ~~Spec 04: previousZone not tracked on drift-engine zone transitions~~ - driftTick now records previousZone whenever zone changes during position-driven transition, not just on manual reclassify
- ~~Spec 12: Search overlay does not close on click-outside or search submission~~ - Added mousedown listener for click-outside close and Enter key closes overlay after cycling per Spec 12 Section 6
